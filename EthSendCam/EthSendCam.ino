// ESP32-CAM Ethernet MJPEG Stream with Timestamps
#include <SPI.h>
...
// include Content-Length + our custom X-Timestamp header (ms since UNIX epoch)
static const char* _STREAM_PART_FMT =
...
void setup() {
  ...
  // Optional sensor tweaks for OV3660
  sensor_t * s = esp_camera_sensor_get();
  if (s->id.PID == OV3660_PID) {
    s->set_vflip(s, 1);
  ...
  // --- Ethernet init ---
  Ethernet.init(2);        // CS pin
  Ethernet.begin(mac);     // DHCP; ignore return if using static IP
  //Ethernet.begin(mac, ip); // if you want fixed IP
...
void stream_h(EthernetClient *ec) {
  ...
  while (ec->connected()) {
    ...
    unsigned long long ts_ms = (unsigned long long)tv.tv_sec * 1000ULL
                              + (tv.tv_usec / 1000ULL);

    // build & send part header
    int hlen = snprintf(part_buf, sizeof(part_buf),
                        _STREAM_PART_FMT,
                        (unsigned int)_jpg_buf_len,
                        ts_ms);
    ec->write((const uint8_t*)part_buf, hlen);
    // send image
    ec->write(_jpg_buf, _jpg_buf_len);
    // send boundary
    ec->write(_STREAM_BOUNDARY, strlen(_STREAM_BOUNDARY));
